...
public class InputManager : BaseManager<InputManager>
{
    ...
    //用于在改键时获取输入信息的委托 只有当Update获取到信息时 才通过委托传递给外部
    ...
    public void ChangeKeyInfo(E_EventType eventType, KeyCode key, InputInfo.E_InputType inputType)
    {
        //初始化
    ...
    {
        //当委托不为空时 证明想要获取到输入信息 传递给外部
        if(isBeginCheckInput)
        {
            //当一个键按下时 遍历所有当前按键信息 得到是谁被按下了
            if (Input.anyKeyDown)
            {
                InputInfo inputInfo = null;
                //我们需要去遍历监听所有键位的按下 来获得对应键位信息
                //键盘
                Array keyCodes = Enum.GetValues(typeof(KeyCode));
                foreach (KeyCode inputKey in keyCodes)
                {
                    //判断到底是谁被按下了 那么就可以得到对应的键盘输入信息
