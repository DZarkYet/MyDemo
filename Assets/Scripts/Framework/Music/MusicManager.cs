...
public class MusicManager : BaseManager<MusicManager>
{
    ...
    public void PlaySound(string name, bool isMp3 = true, bool isLoop = false, bool isSync = false, UnityAction<AudioSource> callBack = null)
    {
        ...
        ABResManager.Instance.LoadResAsync<AudioClip>("sound", name, (clip) =>
        {
            ...
            //如果取出来的音效是之前正在使用的 停止它
            source.Stop();
            source.clip = clip;
            source.loop = isLoop;
            source.volume = soundValue;
            source.Play();
            //存入容器 用于记录 方便之后判断是否停止
            //由于从缓存池中取出对象 有可能取出一个之前正在使用的对象(超上限)
            //所以我们需要判断 容器中没有记录 再去记录 不要重复去添加即可
